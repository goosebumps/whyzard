...
def get_passie_voor_whisky_results(shopname, search_term):
    page = requests.get(
        "https://www.passievoorwhisky.nl/nl/zoeken?controller=search&orderby=position&orderway=desc&search_query=" + search_term)
    soup = BeautifulSoup(page.content, 'html.parser')
    all_products_soup = soup.find_all('div', class_='product-container')
    results = []
    for prod in all_products_soup:
        price = prod.find("div", class_="pro_second_box").find(
            "meta", itemprop="price").get("content")
        name = prod.find(
            "div", class_="pro_second_box").find("h5").find("a").get_text()
        url = prod.find(
            "div", class_="pro_second_box").find("h5").find("a").get("href")
        image = prod.find(
            "div", class_="pro_first_box").find("a").find("img").get("src")
        price = remove_whitespace(price)
        results += [{"name": name, "price": price,
                     "url": url, "shop": shopname, "img": image}]
    return results


def get_d12_results(shopname, search_term):
    page = requests.get(
        "https://drankdozijn.nl/zoeken?zoekterm=" + search_term, cookies=dict(validatie_cookie='true'))
    soup = BeautifulSoup(page.content, 'html.parser')
    all_products_soup = soup.find_all('a', class_='product_top')
    results = []
    for prod in all_products_soup:
        price = prod.find(
            "span", class_="product_aanbieding_prijs").get_text()
        name = prod.find(
            "div", class_="product_title").get_text()
        url = "https://drankdozijn.nl" + prod.get("href")
        image = prod.find(
            "div", class_="product_image").find("img").get("src")
        results += [{"name": name, "price": price,
                     "url": url, "shop": shopname, "img": image}]
    return results
...
def get_theoldpipe_results(shopname, search_term):
    page = requests.get(
        "https://www.theoldpipe.com/nl/search/" + search_term)
    soup = BeautifulSoup(page.content, 'html.parser')
    all_products_soup = soup.find_all(
        'div', class_="product-block-inner")
...
def get_whiskysite_results(shopname, search_term):
    page = requests.get(
        "https://www.whiskysite.nl/nl/search/" + search_term, cookies=dict(age_check='done'))
    soup = BeautifulSoup(page.content, 'html.parser')
    ...
    for prod in all_products_soup:
        ...
        image = prod.find("div", class_="product-block-image").find(
            "img").get("src")
        results += [{"name": name, "price": price,
                     "url": url, "shop": shopname, "img": image}]
    return results
...
def get_shop_results(shopname, search_term):
    shopnames = shop_list.keys() if shopname is 'all' else [shopname]
    reslt = []
    for sn in shopnames:
